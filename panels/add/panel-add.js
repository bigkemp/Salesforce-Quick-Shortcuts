export async function init(importedhandlers){
    ...
    const inputbar = document.getElementById("sqab_p_settings_modalInput");
    inputbar.value = window.location.href.substring(window.location.href.indexOf("com")+3);
    const addlabel = document.getElementById("sqab_p_settings_add_label_input"); 
    let possibleLabel = inputbar.value.split('/');
    addlabel.value = possibleLabel[possibleLabel.length -1];
    const targetList = document.getElementById("sqab_p_settings_targetList");
    ...
    let addSave = document.getElementById("sqab_p_settings_addSave"); 
